...
const OrganizerCreateForm = ({ disabled: _disabled }) => {
  ...
  const handle = {
    create: async (e) => {
      if (_.filter(organizer.organizers, (org) => org.uid === user.id) >= ORGANIZER_PROFILE_LIMIT) {
        enqueueSnackbar(`You can't create more than ${ORGANIZER_PROFILE_LIMIT} organizer.`, {
          variant: "error"
        });
      }
      ...
      organizer
        ...
        .then((res) => {
          if (res.code === "succeed") {
            router.push("/organizer/" + res.id + "/edit");
  ...
  return (
    <Grid container rowSpacing={3}>
      <Grid item xs={12}>
        <Typography variant="h6">Register Event Organizer</Typography>
      </Grid>
      <Grid item xs={12}>
        <InputLabel htmlFor="org-name">Organizer Name</InputLabel>
        <FormHelperText sx={{ mt: 2 }}>
          Controls the publically visible name of this organizer.
        </FormHelperText>
