...
const Page = (props) => {
  ...
  const handleArrange = (e) => {
    ...
    console.info('new matches', newMatches)
    console.info('new games', newGames)
    setMatches([
  ...
  const handlePartyClick = (party, partyWon) => {
    ...
    console.info(newMatches, newGames)
    ...
    <Box sx={{ width: '100%' }}>
      <TabContext value={tab}>
        ...
        <TabPanel value="1">
          ...
          {
            games.length > 0 &&
            <SingleEliminationBracket matches={games} handlePartyClick={handlePartyClick} />
          }
        </TabPanel>
      </TabContext>
    </Box>
